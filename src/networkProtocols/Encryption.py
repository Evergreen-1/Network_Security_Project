...
import random
    #class Encryption
    #description:
    #     This class has the cryptographic primitives for the simplifeid wireguard implementation
class Encryption:
    #Class input varaibles
    SERVER_STATIC_PUBLIC_KEY= None  #Required as input
    client_private_key = None       #Available from website
    ...
    def AEAD_decrypt(self, key, counter, cipher_text, auth_text):
        #Dencrypts the cipher text into plain text
        chacha = ChaCha20Poly1305(key)
        if isinstance(counter, int):
            nonce = (b'\x00' * 4) + counter.to_bytes(8, 'little')
        else:
    ...
    def MixHash(self, message1, message2):  #working
        #This function is not defined in the paper, but will be a useful convenience function 
    ...
    def Timestamp(self, t):
        ...
        microseconds = int((t%1) * 1e6)  #modding for remainder and x 10^6 to get whole number of micro-secs
        return seconds.to_bytes(8, 'big') + microseconds.to_bytes(4, 'big')
    
    def send_prep(self, Construction, Identifier):
        #Creates the initiation header with construction and identifier
    ...
    def encrypt_transport(self, plaintext):
        ...
        # Assembling the transport packet
        ...
        self.N_send += 1  # increment counter
    ...
    def decrypt_transport(self, packet):
        ...
        counter = int.from_bytes(packet[8:16], 'little')
   ...
   #description:
   #     This class extends the Encryption to adopt the mac2 calculations and the cookie handling.
